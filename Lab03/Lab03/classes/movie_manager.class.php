...
class MovieManager {
    ...
    public function search_movie($terms) {
	    ...
	    //check the query, make sure it works fine
        if(!$query){
            return false;
        }
        ...
        //now if there is result, we handle the result
        // create a new array to store movies that we found from  the query
        $movies = array();

        while ($obj = $query->fetch_object()){
            //from each row that we got from the result, we create a new object (movie obj)
            $movie = new Movie(stripslashes($obj->title), stripslashes($obj->rating), stripslashes($obj->release_date),
                stripslashes($obj->director), stripslashes($obj->image), stripslashes($obj->description));
